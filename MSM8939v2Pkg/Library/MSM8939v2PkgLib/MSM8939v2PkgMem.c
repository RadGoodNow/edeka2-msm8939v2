...
STATIC struct MSM8939v2PkgReservedMemory {
  EFI_PHYSICAL_ADDRESS         Offset;
  EFI_PHYSICAL_ADDRESS         Size;
} MSM8939v2PkgReservedMemoryBuffer [] = {
  ...
  )
{
  ...
  Count = sizeof (MSM8939v2PkgReservedMemoryBuffer) / sizeof (struct MSM8939v2PkgReservedMemory);
  while ((NextHob.Raw = GetNextHob (EFI_HOB_TYPE_RESOURCE_DESCRIPTOR, NextHob.Raw)) != NULL)
  {
    if (Index >= Count)
      break;
    if ((NextHob.ResourceDescriptor->ResourceType == EFI_RESOURCE_SYSTEM_MEMORY) &&
        (MSM8939v2PkgReservedMemoryBuffer[Index].Offset >= NextHob.ResourceDescriptor->PhysicalStart) &&
        ((MSM8939v2PkgReservedMemoryBuffer[Index].Offset + MSM8939v2PkgReservedMemoryBuffer[Index].Size) <=
         NextHob.ResourceDescriptor->PhysicalStart + NextHob.ResourceDescriptor->ResourceLength))
    {
      ResourceAttributes = NextHob.ResourceDescriptor->ResourceAttribute;
      ResourceLength = NextHob.ResourceDescriptor->ResourceLength;
      ResourceTop = NextHob.ResourceDescriptor->PhysicalStart + ResourceLength;
      ReservedTop = MSM8939v2PkgReservedMemoryBuffer[Index].Offset + MSM8939v2PkgReservedMemoryBuffer[Index].Size;

      // Create the System Memory HOB for the reserved buffer
      BuildResourceDescriptorHob (
        EFI_RESOURCE_MEMORY_RESERVED,
        EFI_RESOURCE_ATTRIBUTE_PRESENT,
        MSM8939v2PkgReservedMemoryBuffer[Index].Offset,
        MSM8939v2PkgReservedMemoryBuffer[Index].Size
      );
      // Update the HOB
      NextHob.ResourceDescriptor->ResourceLength = MSM8939v2PkgReservedMemoryBuffer[Index].Offset -
